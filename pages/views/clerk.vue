        ...
        <view class="list">
            <view class="item" v-if="userList.length">
                <view class="box" v-for="(item, index) in userList" :key="index" @click="selectF(item)">
                    ...
                    <view class="info">
                        <view class="sub-title">{{item.name}} <text style="color: red;" v-if="item.status==0">（关闭）</text>
						<text v-else>{{' ~ '+item.work_no}}</text></view>
...
export default {
    ...
    methods: {
		...
		getUserList(flag=true){
			...
			app.globalData.ajaxPost('/api/get-staff-list', {keyword:this.search,page:this.page}, (res) => {
			    if (res.code === 200) {
					if(flag){
