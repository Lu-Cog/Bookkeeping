<template>
    <view>
        ...
        <view>
            <view class="h-title">
				<text>{{userType==1?'添加记账':'添加业绩'}}</text>
            ...
            <!-- 记账全局弹框 -->
            <view class="tally">
                <view class="navs" v-if="userType==1">
				...
				<view class="navs" v-else>
					<view :class="'item ' + (form.type == 2 ? 'active' : '')" @tap="setNav" :data-value="2">充值提成</view>
					<view :class="'item ' + (form.type == 1 ? 'active' : '')" @tap="setNav" :data-value="1">消费提成</view>
                    <view :class="'item ' + (form.type == 3 ? 'active' : '')" @tap="setNav" :data-value="3">奖励</view>
					<view :class="'item ' + (form.type == 4 ? 'active' : '')" @tap="setNav" :data-value="4">扣除</view>
                </view>
                <view class="form">
					<view class="formItem">
						<view class="lebal">￥</view>
						<view class="value">
						    <input type="digit" :value="form.money" @input="setMoney" maxlength="6" placeholder-class="plac" placeholder="请输入金额" />
						</view>
					</view>
					<view class="formItem" v-if="userType==1 && form.type==2">
						<view class="lebal">￥</view>
						<view class="value">
				...
				<view class="remark" v-if="userType==1">
				...
				<view class="remark" v-if="form.type!=1 && userType==1">
...
export default {
    ...
    methods: {
        ...
        tijiao() {
            const { date, category_id, name, money } = this.form;
            let code = true;
            ...
            if (!category_id && this.userType==1) {
                uni.showToast({
                    icon: 'none',
                    title: '请选择分类'
                });
			...
			this.form.book_id = app.globalData.book_id
			this.form.staff_id = this.clerk.id
            if (code) {
                // 提交
				let url = ''
				if(this.userType==1){
					url = '/api/set-customer-tally'
				}else{
					url = '/api/set-staff-tally'
				}
                app.globalData.ajaxPost(url, this.form, (res) => {
                    if (res.code === 200) {
                        uni.showToast({
                            icon: 'none',
                            title: '添加成功'
